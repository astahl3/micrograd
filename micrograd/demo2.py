...
Implementation using (0,1) moon classificaiton with sigmoid activation function
in the output layer, tanh for hidden layers
'''
import math
import random
import numpy as np
...
def loss(X, y, model, batch_size=None):
    ...
    scores_sig = [scorei.sigmoid() for scorei in scores]
    
    # custom loss
    losses = [((-y_k + score_k)**2 if abs(-y_k + score_k.data) >= 0.5 else 0) for y_k, score_k in zip(yb, scores_sig)]
    #losses = [(((-y_k+score_k)**2)**(0.5) if abs(-y_k + score_k.data) >= 0.5 else 0) for y_k, score_k in zip(yb, scores)]
    
    # SVM max-margin loss
    #losses = [(1 + -y_k * score_k).relu() for y_k, score_k in zip(yb, scores)]
...
model = net.Multilayer_Perceptron(2, [16,16,1], 'tanh') # 2 layer network
print(model)
N = 200
lRate0 = 0.1
for k in range(0,N):
